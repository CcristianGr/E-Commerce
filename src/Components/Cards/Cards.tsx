...
export const Cards = ({ data = [] }: CardsProps) => {
    ...
    const addProductsToCard = (productData: any) => {
        ...
        <div className="grid grid-cols-4 gap-4 p-4">
            {
                data?.map((item: any, index: number) => (
                    <div className=" w-56 h-60 rounded-lg p-4 flex flex-col justify-" key={item.id ?? index}>
                        <figure className="relative w-full h-4/5">
                            <div 
                                ...
                                onClick={(e) => { e.stopPropagation(); addProductsToCard(item); }}>
